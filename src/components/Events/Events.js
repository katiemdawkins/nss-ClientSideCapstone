...
export const DisplayEvents = () => {
    ...
    return(
        ...
        {
            <div>
                <input placeholder="Search for an event topic" onChange={event=> setQuery(event.target.value)}></input>
            {
                events.filter(event=>{
                    if(query === ''){
                        return event
                            ...
                            <p>Event Topic: {event.eventTopic.name}</p>
                            {
                                userProfiles.map((userProfile)=>{
                                    if(userProfile.userId === event.userId){
                                        return <p>Hosted by: <Link to={`/connect/${userProfile.id}`}>{event.user.firstName} {event.user.lastName}</Link></p>
                                    }
                                    }
                                )
                            }
